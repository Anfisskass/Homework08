...
from view import (get_employee, get_changes, get_id, get_first_name, get_last_name, show_employee_info,
                    get_phone_number, get_position, get_salary, get_salary_range, no_employee_error, show_search_menu)


#fields = ["id", "last_name", "position", "phone_number", "salary"]

def read_csv() -> list:
    employee = []
    with open(Path.cwd() / 'database.csv', 'w', encoding='utf-8') as fin:
...
def read_json() -> list:
    employee = []
    with open(Path.cwd() / 'database02.json', 'w', encoding='utf-8') as fin:
        for line in fin:
...
def find_employee_by_salary_range(employees: list) -> list:
    result = []
